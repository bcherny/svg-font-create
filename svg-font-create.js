...
var DOMParser, SvgPath, config, convert, cssTemplate, defaults, execSync, fs, generate, glob, hiddenRules, htmlTemplate, isHidden, parse, path, rgxAcronym, rgxName, rgxUnicode, sassTemplate, svgTemplate, util, _;
...
isHidden = function(element) {
  return element.getAttribute('style').split(';').filter(function(string) {
    return string;
  }).map(function(string) {
    return string.split(':');
  }).some(function(rule) {
    return hiddenRules[rule[0]] === rule[1];
  });
};
