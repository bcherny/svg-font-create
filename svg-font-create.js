...
var DEFAULT_CONFIG, DOMParser, config, convert, cssTemplate, execFile, execSync, fs, fstools, glob, htmlTemplate, parseSvgImage, path, rgxAcronym, rgxFiles, rgxName, rgxUnicode, sassTemplate, svgFontTemplate, svgImageTemplate, util, _;
...
convert = function(args) {
  var font, fontHeight, svgoConfig, tmpDir;
  if (args.name != null) {
  ...
  fstools.walkSync(args.input_dir, rgxFiles, function(file) {
    var glyph, scale, svgOut, transform;
    ...
    svgOut = svgImageTemplate({
      ...
      width: glyph.width,
      d: glyph.d,
      transform: transform + (glyph.transform != null ? " " + glyph.transform : transform)
    });
    return fs.writeFileSync(path.join(tmpDir, path.basename(file)), svgOut, 'utf8');
  });
  console.log('Optimizing images');
  svgoConfig = args.svgo_config != null ? path.resolve(args.svgo_config) : path.resolve(__dirname, 'svgo.yml');
  return execFile(path.resolve(__dirname, './node_modules/.bin/svgo'), ['-f', tmpDir, '--config', svgoConfig], function(err) {
    var eot, glyphs, opts, svg, ttf, woff;
    if (err) {
      console.error(err);
      process.exit(1);
    }
    ...
    return _.forEach({
      ...
      'Cleaning up temporary files': function() {
        return fstools.removeSync(tmpDir);
      },
      'Done!': function() {}
    }, function(fn, message) {
      console.log(message);
